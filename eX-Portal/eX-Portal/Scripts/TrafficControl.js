...
var isDataLoadingInProgress=false;
...
function ChartDataAdd(Position) {
  ...
  for (; ChartIndex <= iPosition; ChartIndex++) {
    ...
    theChartSeries[1].addPoint([categories, DataItem.MediumSpeed], false, isRemovePoints);
    theChartSeries[0].addPoint([categories, DataItem.NumberOfCar], true, isRemovePoints);
  }
}
...
function getmoredata() {
    ...
    $.ajax({
        ...
        success: function (Row) {
            alert(Row);
            ChartData = Object.keys(Row).map(function (_) { return Row[_]; });
