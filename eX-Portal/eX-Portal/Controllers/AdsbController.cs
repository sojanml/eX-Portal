...
namespace eX_Portal.Controllers {
  public class AdsbController : Controller {
    ...
    public List<PilotFlight> GetActivePilots() {
      ...
      var Pilots = from drone in ctx.MSTR_Drone.Where(x => x.FlightTime > dt)
                    join flight in ctx.DroneFlight on drone.LastFlightID equals flight.ID
                    join pilot in ctx.MSTR_User on flight.PilotID equals pilot.UserId
                    select new PilotFlight {
                      FlightID = flight.ID,
    ...
    public List<PilotFlight> GetActivePilotsRegion(string Coordinates) {
      var flUSer = GetActivePilots();
      //var ActPF = flUSer.ActiveFlight;
      List<PilotFlight> ActRegionPilot = new List<PilotFlight>();
      foreach (PilotFlight acp in flUSer) {
